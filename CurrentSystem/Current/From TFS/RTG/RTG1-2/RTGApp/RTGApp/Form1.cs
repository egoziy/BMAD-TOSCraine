...
namespace RTGApp
{
    public partial class FrmMap01 : Form
    {
        ...
        private void DG_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (DG.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString() != "" & Convert.ToInt32(e.ColumnIndex) >= 1)
                {
                    ...
                    string sql = "SELECT   SUBSTRING(dbo.TB_Location.LocationCode, 5, 1) " +
                ...
                " THEN '' ELSE OrderDesc END + case when CO_Containers.MarksNumbers <> '' then '(' + CO_Containers.MarksNumbers + ')' else '' end AS  '" + StrLocation + "'  " +
                ...
                " ORDER BY dbo.TB_Location.LocationCode DESC ";
                    Utils.WriteLog(sql);
                    DataTable dt2 = Con1.ReturnDT(sql);
            ...
            catch(Exception ex)
            {
                ...
                Utils.WriteLog(ex.ToString(), errnum.ToString());
            }
                ...
                if (e.ColumnIndex > 0)
                {
                    ...
                    if (FirstComplete == false)
                    {
                        ...
                        if (DG.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString() != "" & Convert.ToInt32(e.ColumnIndex) >= 1)
                        {
                            ...
                            string sql = "SELECT     dbo.TB_Location.Container, dbo.CO_ContainerProfile.ContainerLength, dbo.CP_Deal.HandlingTypeCode" +
                            " FROM         dbo.TB_Location INNER JOIN " +
        ...
        static Color ReturnPermitColor(string Container)
        {
            ...
            try
            {
                string sql = "SELECT     dbo.CP_Order.Container  " +
              ...
              " < 200) AND (dbo.CP_Order.Container = '" + Container + "')";
                Utils.WriteLog(sql);
                DataTable dt5 = Con3.ReturnDT(sql);
        ...
        public bool MappingSquare()
        {
            ...
            try
            {
                ...
                if (BlocDef == "BOND1")
                {
                    string sql = "SELECT LEFT(dbo.CO_Containers.LocationCode, 3) AS ColumnStr, " +
                    ...
                    Utils.WriteLog(sql);
                    DataTable dt2 = Con.ReturnDT(sql);
            ...
            try
            {
                ...
                for (int i = 0; i <= dt.Rows.Count - 1; i++)
                {
                    for (int j = 1; j <= dt.Columns.Count - 1; j++)
                    {
                        ...
                        string sql = "SELECT   COUNT(dbo.CO_Containers.Container) AS Counter " +
                      " FROM     dbo.TB_Location INNER JOIN " +
                      " dbo.CO_Containers ON dbo.TB_Location.Container = dbo.CO_Containers.Container INNER JOIN " +
