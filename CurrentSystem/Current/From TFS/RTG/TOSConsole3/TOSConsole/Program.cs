    ...
    class Program
    {
        ...
        START:
            ...
            try
            {
             ...
             //   IPAddress localAddr = IPAddress.Parse("192.6.1.8");
                ...
                while (true)
                {
                    Console.Write("Waiting for a connection GOLD3 ... ");
                ...
                Outer:
                    ...
                    try
                    {
                        ...
                        DataTable dt = CC.ReturnDT("SELECT Message FROM dbo.RG_B3 WHERE  CHE = 'GOLD3'  AND A3Date IS NULL  AND (PickDate IS NULL)  UNION ALL SELECT  MessageCancel FROM dbo.RG_B3 WHERE   (CHE = 'GOLD3') AND (MessageCancel IS NOT NULL) AND (CancelDate IS NULL)");



                        if (dt.Rows.Count == 0)
                        {
                            Console.Write("Waiting BROADCAST  GOLD3 ... \n");
                        ...
                        while ((i = stream.Read(bytes, 0, bytes.Length)) != 0)
                        {
                            ...
                            if (data.Substring(0, 2) == "??")
                            {
                                                                     ...
                                                                     "  HBRowNumber ='" + data.Substring(33, 3) + "', " +
                                                                      ...
                                                                      " CraneStatus ='" + data.Substring(40, 2) + "', " +
                                                                       "  GPSStatus ='" + data.Substring(42, 2) + "', " +
                                                                        ...
                                                                        " TwistLock ='" + data.Substring(77, 1) + "' WHERE CHE = 'GOLD3'");
                                ...
                                if (data.Substring(4, 2) == "A2")
                                {
                                    ...
                                    if (data.Substring(14, 2) == "03")  // PICK
                                    {
                                        ...
                                        if (data.Substring(89, 1).ToString() == "G" || data.Substring(89, 1).ToString() == "T")
                                        {
                                            ...
                                            dt = CC.ReturnDT("SELECT Count(*) as Counter FROM  dbo.RG_Container WHERE CHE = 'GOLD3'");
                                            if (Convert.ToInt32(dt.Rows[0][0].ToString()) == 0)
                                            {
                                                dt = CC.ReturnDT("UPDATE dbo.TB_Parameters SET ContainerPick3 ='TRUE'");
                                            }
                                        }


                                        // Counter='" + data.Substring(12, 2) + "'  AND
                                        dt = CC.ReturnDT("UPDATE RG_B3 SET PickDate = GetDate() WHERE (CounterID =(SELECT MAX(CounterID) AS Expr1 FROM dbo.RG_B3 AS RG_B3_1 WHERE  CHE = 'GOLD3')   AND CHE = 'GOLD3')"); // WHERE PickDate IS NULL AND  LiftBlockName = '" + data.Substring(78, 5) + "' AND LiftBayNumber = '" + data.Substring(86, 3) + "' AND LiftRowNumber =  '" + data.Substring(89, 1) + "' AND LiftHeight = '" + data.Substring(92, 1) + "' AND Len = '" + data.Substring(114, 2) + "' AND CHE = 'GOLD1'");
                                        ...
                                        if (dt.Rows.Count > 0)
                                        {
                                            ...
                                            dt = CC.ReturnDT("SELECT OperatorID, TruckType FROM  dbo.RG_B3 WHERE (CounterID =(SELECT MAX(CounterID) AS Expr1 FROM dbo.RG_B3 AS RG_B3_1 WHERE  CHE = 'GOLD3')   AND CHE = 'GOLD3')");
                                    ...
                                    if (data.Substring(14, 2) == "04")  // PLACE
                                    {
                                        ...
                                        // Counter='" + data.Substring(12, 2) + "'  AND
                                        dt = CC.ReturnDT("UPDATE RG_B3 SET PlaceDate = GetDate() WHERE (CounterID =(SELECT MAX(CounterID) AS Expr1 FROM dbo.RG_B3 AS RG_B3_1 WHERE  CHE = 'GOLD3')   AND CHE = 'GOLD3')");// WHERE PlaceBlockName = '" + data.Substring(78, 5) + "' AND PlaceBayNumber = '" + data.Substring(86, 3) + "' AND PlaceRowNumber =  '" + data.Substring(89, 1) + "' AND PlaceHeight = '" + data.Substring(92, 1) + "' AND Len = '" + data.Substring(114, 2) + "' AND CHE = 'GOLD1'");  // , FinishDate = GetDate()

                                        dt = CC.ReturnDT("SELECT COUNT(Container) AS Counter FROM     dbo.RG_Container WHERE CHE = 'GOLD3'");

                                        if (Convert.ToInt16(dt.Rows[0][0].ToString()) > 0)
                                        {
                                            dt = CC.ReturnDT("SELECT Container, OperatorID FROM  dbo.RG_Container WHERE CHE = 'GOLD3'");
                                            ContainerPick = dt.Rows[0][0].ToString();
                                            OperatorIDPick = dt.Rows[0][1].ToString();

                                            dt = CC.ReturnDT("DELETE dbo.RG_Container WHERE CHE = 'GOLD3'");
                                        ...
                                        if (ContainerPick.Trim().Length == 11)
                                        {
                                            ...
                                            dt = CC.ReturnDT("UPDATE dbo.TB_Location SET Container  = Null , CHE  = 'GOLD3'  WHERE Container = '" + ContainerPick + "'");

                                            if (data.Substring(89, 1).ToString() == "T")
                                            {
                                            ...
                                            {
                                                dt = CC.ReturnDT("UPDATE dbo.TB_Location SET Container  =  '" + ContainerPick + "', CHE = 'GOLD3'  WHERE    LocationCode  = '" + data.Substring(86, 3) + data.Substring(89, 1) + data.Substring(92, 1) + "' AND BlocCode = '" + data.Substring(78, 5) + "'");
                                ...
                                if (data.Substring(4, 2) == "A3")
                                {
                                    ...
                                    dt = CC.ReturnDT("UPDATE RG_B3 SET CancelDate  = GetDate() WHERE  A3Date IS NOT NULL AND  Counter='" + data.Substring(6, 2) + "' AND CHE = 'GOLD3'");
                                    dt = CC.ReturnDT("UPDATE RG_B3 SET A3Date  = GetDate() WHERE  A3Date IS  NULL AND  Counter='" + data.Substring(6, 2) + "' AND CHE = 'GOLD3'");
        ...
        public static void WriteLog(string strLog)
        {
            try
            {
                ...
                string logFilePath = @"\\Broadcast\Logs\RTG\RTG3\TOSConsole3\Log_" + System.DateTime.Now.DayOfYear.ToString() + "." + "txt";
