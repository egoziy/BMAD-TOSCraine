    ...
    public partial class FrmLogin : Form
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt = Con1.ReturnDT("SELECT COUNT(*) AS Counter " +
                                         ...
                                         "dbo.HR_Emp ON dbo.SC_Users.EmpID = dbo.HR_Emp.EmpID INNER JOIN " +
                                         "sys.syslogins ON sys.syslogins.loginname COLLATE SQL_Latin1_General_CP1_CI_AS = dbo.HR_Emp.LoginName " +
                                         "WHERE (dbo.SC_Users.UserGroupCode = 22) AND (dbo.HR_Emp.VirtualEmp = 0) AND " +
                                         "(pwdcompare('" + Password.Text + "', sys.syslogins.password) = 1) AND (sys.syslogins.loginname = N'" + LoginName.Text + "')");
            if (dt.Rows.Count > 0)
            {
        ...
        {
                this.CHE.Text = "GOLD3";
                   this.LiftBlockName.Text = "Bond3";
                   this.LiftBlockName.Enabled = false;

        }

        private void LiftBlockName_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void LoginName_SelectedIndexChanged_1(object sender, EventArgs e)
        {
