...
namespace RTGApp
{
   public partial class FrmMap01 : Form
   {
            ...
            for (int j = 0; j <= dt.Columns.Count - 1; j++)
            {
               if (j < 20)
               {
                  if (dt.Rows[i][j].ToString() != "0")
                  {
                     ...
                     {
                        DG.Rows[i].Cells[j].Value = dt.Rows[i][j].ToString();
               ...
               else
               {
                  ...
                  {
                     DG.Rows[i].Cells[j+1].Value = dt.Rows[i][j].ToString() + " | " + dt.Rows[i][j+2].ToString();
                     if (int.Parse(dt.Rows[i][j].ToString()) > int.Parse(dt.Rows[i][j+2].ToString()))
                        DG.Rows[i].Cells[j+1].Style.ForeColor = ReturnColor(dt.Rows[i][j].ToString());
                     else
                        DG.Rows[i].Cells[j + 1].Style.ForeColor = ReturnColor(dt.Rows[i][j+2].ToString());
               ...
               {
                  DG.Rows[i].Cells[j].Style.ForeColor = ReturnColor(dt.Rows[i][j].ToString());
      ...
      private void button1_Click(object sender, EventArgs e)
      {
         ...
         string s = "SELECT     TOP (100) PERCENT dbo.CO_Containers.Container, dbo.CO_ContainerProfile.ContainerLength, dbo.CO_ContainerProfile.ContainerTypeCode, " +
         ...
         str += "לקוח: ";
         str += dt2.Rows[0][7].ToString() + (char)10 + (char)10;

         str += "עבודה: ";
         str += dt2.Rows[0][8].ToString() + (char)10 + (char)10;

         str += "התרה: ";
         str += dt2.Rows[0][9].ToString() + (char)10 + (char)10;
