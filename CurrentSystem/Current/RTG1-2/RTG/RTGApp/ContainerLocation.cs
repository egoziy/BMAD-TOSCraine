...
namespace RTGApp
{
    public partial class ContainerLocation : Form
    {
       ...
       //     dt = Con1.ReturnDT("UPDATE CO_Containers set LocationCode = '" + this.Location1.Text + this.Location2.Text + this.Location3.Text + "'  WHERE  (EntranceDate IS NOT NULL OR RegisterDate IS NOT NULL) AND (ExitDate IS NULL) AND (Container = '" + this.cboContainerLetter.Text + this.ContainerNumber.Text + "')");
       //      dt = Con1.ReturnDT("UPDATE dbo.TB_Parameters SET RefreshMapRTG ='TRUE',RefreshMapRTG2 ='TRUE',RefreshMapRTG3 ='TRUE',RefreshMapRTG4 ='TRUE'");
        ...
        public static void WriteLog(string strLog, string LogFileName = "")
        {
           ...
           if (LogFileName != "")
              logFilePath = @"LogFileName-" + System.DateTime.Today.ToString("MM-dd-yyyy") + "." + "txt";
           ...
           using (FileStream fileStream = new FileStream(logFilePath, FileMode.Append))
           {
              ...
              {
                 log.WriteLine(string.Format("version 2021-06-10 {0} : {1} ", DateTime.Now, strLog));
              }
           }
        }
